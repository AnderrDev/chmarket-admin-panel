...
export default function Discounts() {
  const { discounts, loading, deleteDiscount, toggleDiscountStatus } = useDiscounts()
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <div className="mt-1 flex items-center text-sm text-gray-500">
                        <span className="truncate">{getDiscountValue(discount)}</span>
                        <span className="mx-2">•</span>
                        <span>Min: {formatCurrency(discount.min_order_cents)}</span>
                        <span className="mx-2">•</span>
                      ...
                      <div className="mt-1 text-sm text-gray-500">
                        ...
                        {discount.end_at && (
                          <span>
                            Hasta: {formatDate(discount.end_at)}
                          </span>
                        )}
      ...
      {discounts.length > 0 && (
        <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white shadow rounded-lg p-4">
            <div className="text-sm font-medium text-gray-500">Total Cupones</div>
            <div className="text-2xl font-bold text-gray-900">{discounts.length}</div>
          </div>
          <div className="bg-white shadow rounded-lg p-4">
            <div className="text-sm font-medium text-gray-500">Cupones Activos</div>
            <div className="text-2xl font-bold text-green-600">
              {discounts.filter(d => d.is_active).length}
            </div>
          </div>
          <div className="bg-white shadow rounded-lg p-4">
            <div className="text-sm font-medium text-gray-500">Cupones Inactivos</div>
            <div className="text-2xl font-bold text-gray-600">
              {discounts.filter(d => !d.is_active).length}
            </div>
          </div>
          <div className="bg-white shadow rounded-lg p-4">
            <div className="text-sm font-medium text-gray-500">Tipos Diferentes</div>

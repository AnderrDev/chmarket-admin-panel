...
Deno.serve(async req => {
  ...
  try {
    ...
    // Si pago falló → NO liberar reservas (mantener para reintentos)
    if (status === 'REJECTED' || status === 'CANCELLED') {
      console.log(
        '❌ Pago falló, manteniendo reservas para posibles reintentos'
      );
      // Las reservas se mantienen hasta que:
      // 1. Pago sea exitoso (APPROVED) → se confirman
      // 2. Orden se cancele manualmente → se liberan
      // 3. Reserva expire (30 min) → se liberan automáticamente
    }
    ...
    if (status === 'APPROVED' && order.status !== 'PAID') {
      ...
      for (const it of items ?? []) {
        try {
          console.log(
            `🔄 Decrementando stock: variant ${it.variant_id}, qty ${it.quantity}`
          );
          ...
          console.error(
            `❌ Error decrementando inventario para variant ${it.variant_id}:`,
            error
          );
        }
      }

      // Confirmar reservas de cupones (mover de coupon_reservations a order_discounts)
      ...
      if (reservations && reservations.length > 0) {
        console.log('🎫 Confirmando reservas de cupones:', reservations.length);

        for (const reservation of reservations) {
          try {
            await supabase.rpc('confirm_coupon_reservation', {
              p_order_id: order.id,
              p_discount_id: reservation.discount_id,
            });
            console.log(
              `✅ Reserva confirmada para cupón ${reservation.discount_id}`
            );
          } catch (error) {
            console.error(
              `❌ Error confirmando reserva para cupón ${reservation.discount_id}:`,
              error
            );
          }
        }

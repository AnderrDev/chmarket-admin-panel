  ...
  try {
        ...
        if (!discountData || Object.keys(discountData).length === 0) {
          return new Response(
            JSON.stringify({
              ...
              details:
                'El cuerpo de la solicitud debe contener los datos del cupón',
            }),
            {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              status: 400,
        ...
        if (!discountId || !discountData) {
          return new Response(
            JSON.stringify({
              ...
              details:
                'Se requiere discountId y discountData para actualizar el cupón',
            }),
            {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              status: 400,
        ...
        if (!discountId || isActive === undefined) {
          return new Response(
            JSON.stringify({
              error: 'Datos requeridos faltantes',
              details:
                'Se requiere discountId e isActive para cambiar el estado del cupón',
            }),
            {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              status: 400,
    ...
    if (method === 'GET' && path === 'active') {
      try {
        ...
        console.error(
          'Unexpected error fetching active discounts:',
          fetchError
        );
        return new Response(
          JSON.stringify({
